  ...
  <div>
    ...
    <div>
      <el-checkbox-group v-if="problem.multiple" v-model="answer" @change="updateAnswer">
        <el-checkbox v-for="(value, index) in choice" :key="index" :label="index" class="choice-option">
          {{ String.fromCharCode(65 + index) }}. {{ value }}
        </el-checkbox>
      </el-checkbox-group>
      <el-radio-group v-else v-model="answer[0]" @change="updateAnswer">
        <el-radio v-for="(value, index) in choice" :key="index" :label="index" class="choice-option">
          {{ String.fromCharCode(65 + index) }}. {{ value }}
        </el-radio>
      </el-radio-group>
    </div>
